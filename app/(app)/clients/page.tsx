...
const PAGE_SIZE = 100;
// v1: reorderable client columns persisted here. Name is pinned first, so only
// the optional columns are stored.
...
const DEFAULT_CLIENT_ORDER: ClientColKey[] = [
  'state', 'stage', 'signed', 'countdown', 'rep', 'gross', 'projection', 'source', 'email', 'phone',
];

// Text columns read better ascending; scores/dates read better with the biggest
// or soonest first — matching the contacts grid's defaults.
const ASC_FIRST = new Set(['name', 'state', 'source', 'email', 'phone']);

interface ClientColumn {
  label: string;
  /** DB column the server sorts on; omit for non-sortable (interactive) columns. */
  sortKey?: string;
  adminOnly?: boolean;
...
/** Clients view: reorderable, sortable columns; stages, countdown, revenue. */
export default function ClientsPage() {
  ...
  const columns: Record<ClientColKey, ClientColumn> = {
    ...
    rep: { label: 'Rep Score', sortKey: 'reputation_score', cellClass: 'font-mono', render: (c) => c.reputation_score ?? '' },
    gross: {
      label: 'Gross',
    ...
    projection: {
      ...
      sortKey: 'revenue_projection',
      adminOnly: true,
      cellClass: 'font-mono text-green-700',
      render: (c) => (c.revenue_projection > 0 ? `$${Number(c.revenue_projection).toLocaleString()}` : ''),
    },
              ...
              {visibleCols.map((key) => {
                ...
                return (
                  <th
                    ...
                    className={`grid-th ${col.sortKey ? 'cursor-grab hover:text-gray-700 active:cursor-grabbing' : 'cursor-grab active:cursor-grabbing'} ${
