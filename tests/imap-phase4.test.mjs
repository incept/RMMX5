...
test('append_sent write-back builds a MIME and appends to the Sent folder', async () => {
  ...
  assert.match(sync, /'append_sent' \| 'reconcile'/);
  ...
  // Idempotent: a retried append doesn't add a second copy.
  assert.match(sync, /function sentAlreadyHas/);
});
...
test('interactive 1:1 sends opt into Sent; bulk + sequences do not', async () => {
  ...
  assert.match(send, /enqueueImapAppendSent\(row\.id\)/);
