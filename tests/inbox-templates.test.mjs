...
test('the template editor is a shared component that writes and sanitizes', async () => {
  ...
  // Owns create / update / delete through admin-gated server endpoints.
  assert.match(editor, /\/api\/email\/templates\/\$\{form\.id\}/);
  assert.match(editor, /method: form\.id \? 'PATCH' : 'POST'/);
