...
test('the two transports carry different bodies so tracking never double-counts', async () => {
  ...
  assert.match(send, /const emailitHtml = opts\.html \+ signature/);
...
test('the Emailit webhook records email.loaded/clicked into the shared counter', async () => {
  ...
  // Maps by the meta row id first, recipient as fallback.
  assert.match(route, /emailObj\?\.meta\?\.message_row_id/);
  assert.match(route, /ilike\('email', String\(rcpt\)\)/);
  ...
  // recorded.
  assert.match(route, /track_email_event_bounded/);
  ...
  assert.match(route, /stopEnrollmentsFor\(stopContact, isClick \? 'click' : 'open'\)/);
